...
def run(playwright: Playwright) -> None:
    ...
    page.goto("https://www.werkenbijssc-ons.nl/vacatures")
    content = []
    for job in page.get_by_role('link').all():
        if job.get_attribute('href').startswith('/vacatures/'):
            job.click()
            job_title = page.locator('h3.uppercase').text_content()
            employer = 'ONS Zwolle'
            jobText = page.locator('article.prose').first.text_content()
            jobUrl = page.url
            page.go_back()

            if not jobUrl in scrapedJobsUrls:
                content.append([now, job_title, employer,jobUrl, jobText, 'ONS'])
            else:
