...
def run(playwright: Playwright) -> None:
    ...
    page.goto("https://www.kampen.nl/vacatures")
    content = []
    vacancyList = page.locator('div.block_vacancy_block_list')
    for job in vacancyList.locator('li').all():

        jobTitle = job.locator('h3').text_content()
        page.goto('https://kampen.nl'+job.locator('a').get_attribute('href'))

        
        employer = 'Gemeente Kampen'
        jobText = page.locator('div.MuiGrid-item').first.text_content()
        jobUrl =  page.url
        source = 'Gemeente Kampen'
